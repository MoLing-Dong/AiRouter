...
class AliQwenAdapter(BaseAdapter):
    ...
    async def chat_completion(self, request: ChatRequest) -> ChatResponse:
        ...
        try:
            ...
            raise Exception(f"千问API错误: {str(e)}")

        except Exception as e:
            response_time = time.time() - start_time
            self.update_metrics(response_time, False)
            self.health_status = HealthStatus.UNHEALTHY
            raise Exception(f"千问适配器错误: {str(e)}")

    async def stream_chat_completion(self, request: ChatRequest):
        """执行千问流式聊天完成请求 - 使用OpenAI库"""
        start_time = time.time()
        ...
        try:
            ...
            raise Exception(f"千问流式API错误: {str(e)}")

        except Exception as e:
            response_time = time.time() - start_time
            self.update_metrics(response_time, False)
            self.health_status = HealthStatus.UNHEALTHY
            raise Exception(f"千问流式适配器错误: {str(e)}")
    ...
    async def health_check(self) -> HealthStatus:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"AliQwenAdapter健康检查成功 - 测试聊天完成")
            ...
            # 如果都失败了，标记为降级
            logger.info(f"AliQwenAdapter健康检查失败，标记为降级")
            self.health_status = HealthStatus.DEGRADED
    ...
    async def create_embedding(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise Exception(f"千问嵌入创建错误: {str(e)}")
    ...
    async def list_models(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise Exception(f"千问模型列表获取错误: {str(e)}")
