...
class SQLModelDatabaseService:
    ...
    def get_model_by_name(self, model_name: str, is_enabled: Optional[bool] = None) -> Optional[LLMModel]:
        """根据名称获取模型"""
        with self.get_session() as session:
    ...
    def update_model(self, model_id: int, model_data: Dict[str, Any]) -> Optional[LLMModel]:
        """更新模型"""
        with self.get_session() as session:
    ...
    def create_provider(self, provider_data: ProviderCreateRequest) -> LLMProvider:
        ...
        with self.get_session() as session:
            provider = LLMProvider(
                ...
                third_party_endpoint=provider_data.third_party_endpoint,
    ...
    def get_models_with_providers(self, is_enabled: Optional[bool] = None) -> List[Dict[str, Any]]:
        """获取模型及其提供商信息"""
        with self.get_session() as session:
            statement = (
    ...
    def create_model_provider_association(
        ...
        with self.get_session() as session:
            ...
            logger.info(f"Created association: model_id={model_id}, provider_id={provider_id}")
    ...
    def batch_update_model_provider_metrics(self, updates: List[Dict[str, Any]]) -> bool:
        """批量更新模型-提供商指标"""
        try:
            with self.get_session() as session:
    ...
    def get_api_keys_for_provider(self, provider_id: int, is_enabled: Optional[bool] = None) -> List[LLMProviderApiKey]:
        """获取提供商的API密钥"""
        with self.get_session() as session:
            statement = select(LLMProviderApiKey).where(LLMProviderApiKey.provider_id == provider_id)
            if is_enabled is not None:
    ...
    def get_best_api_key(self, provider_id: int) -> Optional[LLMProviderApiKey]:
        ...
        with self.get_session() as session:
            statement = (
                ...
                .order_by(LLMProviderApiKey.is_preferred.desc(), LLMProviderApiKey.weight.desc())
            )
    ...
    def get_model_config_from_db(self, model_name: str) -> Optional[Dict[str, Any]]:
        ...
        with self.get_session() as session:
            ...
            statement = (
                ...
                .join(LLMProviderApiKey, LLMProvider.id == LLMProviderApiKey.provider_id)
                .where(
                ...
                .order_by(LLMModelProvider.priority.desc(), LLMModelProvider.weight.desc())
            )
    ...
    def get_database_statistics(self) -> Dict[str, Any]:
        ...
        with self.get_session() as session:
            ...
            # 统计健康状态
            healthy_count = len(session.exec(
                select(LLMModelProvider).where(LLMModelProvider.health_status == HealthStatus.healthy)
            ).all())
